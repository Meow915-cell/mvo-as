        ...
        <div class="main-content">
            ...
            <div id="calendar-header-controls" class="flex items-center justify-between mb-4">
                <div class="flex gap-2">
                    <button id="prev-button" class="btn-sm-icon-primary"><svg
                            xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                            stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                            class="lucide lucide-chevron-left-icon lucide-chevron-left">
                            <path d="m15 18-6-6 6-6" />
                        </svg></button>
                    <button id="next-button" class="btn-sm-icon"><svg xmlns="http://www.w3.org/2000/svg"
                    ...
                    <button id="today-button" class="btn-sm">
            ...
            <section>
                <form class="form grid gap-4" action="../actions/manage_restricted_date.php" method="POST">
                    ...
                    <div class="grid gap-3">
                        ...
                        <input type="date" name="restricted_date" id="restricted_date" readonly required class="text-foreground" />
                    </div>
                    ...
                    <div id="time-input-group" class="time-input-group grid gap-3">
                        ...
                        <div class="grid gap-2">
                            <label for="start_time_slider" class="text-sm">Start Time: <span id="start_readout" class="font-semibold text-primary">09:00 AM</span></label>
                            <input
                              type="range"
                              class="input w-full time-slider"
                        ...
                        <div class="grid gap-2">
                            <label for="end_time_slider" class="text-sm">End Time: <span id="end_readout" class="font-semibold text-primary">04:00 PM</span></label>
                            <input
                              type="range"
                              class="input w-full time-slider"
                              id="end_time_slider"
                    ...
                    <footer class="flex justify-between gap-2 mt-4">
                        <button type="button" id="deleteBtn" class="btn btn-danger btn-outline" style="display:none;">Delete</button>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                dayCellDidMount: function(arg) {
                    ...
                    const dayEvents = events.filter(e => e.start && e.start.startsWith(dateStr) && !e.allDay);
                    ...
                    const WORK_START = SLIDER_START_MIN, WORK_END = SLIDER_END_MIN;
                    const toMin = t => {
                    ...
                    let cur = WORK_START;
                    merged.forEach(b => {
                        if (cur < b.start) available.push({ start: cur, end: b.start });
                        cur = Math.max(cur, b.end);
                    });
                    if (cur < WORK_END) available.push({ start: cur, end: WORK_END });

                    const label = document.createElement('div');
                    label.className = 'time-label';
                    label.innerHTML += '⛔ ' + merged.map(b => 
                        `${minToLabel(Math.max(b.start,WORK_START))}-${minToLabel(Math.min(b.end,WORK_END))}`).join(', ');
                    label.innerHTML += '<br>';
           ...
           document.getElementById('deleteBtn').onclick = () => {
                ...
                if (!confirm('Are you absolutely sure you want to delete this restricted date? This action cannot be undone.')) return;
